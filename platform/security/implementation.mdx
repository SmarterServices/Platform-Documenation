...
The Security Token Service mints **short-lived, narrowly-scoped, revocable** credentials from a longer-lived one — the AWS STS analogue. Use it whenever a credential has to be handed to a less-trusted context (a browser embed, an on-behalf-of call) so the original broad credential never leaves your backend.
...
For the SmarterElements browser flow (passing the token to the SDK and silent refresh), see [SmarterElements → Authentication & Tokens](/smarter-elements/authentication).
...
// Bring your own cache store (any get/set-with-ttl implementation).
...
const iam = new IamService({
  ...
  cache: {
    ...
    set: async (key, value, ttl) => {
      await cacheStore.set(key, JSON.stringify(value), ttl);
    }
  }
});
```
